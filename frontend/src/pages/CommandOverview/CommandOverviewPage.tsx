...
import { alertService, dashboardService, analyticsService } from '../../services';
import { IntelligenceAlert, PriorityNetworkItem } from '../../types';

export const CommandOverviewPage: React.FC = () => {
  const navigate = useNavigate();
  const [alerts, setAlerts] = useState<IntelligenceAlert[]>([]);
  const [networks, setNetworks] = useState<PriorityNetworkItem[]>(mockPriorityNetworks);
  const [stats, setStats] = useState({
    activeInvestigations: 142,
    priorityNetworksCount: 8,
    emergingSignals: 84,
    crossCaseLinks: 29,
  });
  ...
  useEffect(() => {
    ...
    async function loadCommandData() {
      try {
        const [aRes, netRes, dRes] = await Promise.all([
          alertService.getAlerts(),
          analyticsService.getPriorityNetworks(),
          dashboardService.getDashboardData(),
        ]);
        if (isMounted) {
          if (aRes && aRes.length > 0) setAlerts(aRes);
          if (netRes && netRes.length > 0) setNetworks(netRes);
          if (dRes && dRes.stats) {
            setStats({
              activeInvestigations: dRes.stats.activeInvestigations || 142,
              priorityNetworksCount: netRes.length || 8,
              emergingSignals: dRes.stats.intelligenceSignals || 84,
              crossCaseLinks: 29,
            });
          }
        }
      } catch (err) {
        console.warn('Failed to load command overview data:', err);
      }
    }
    loadCommandData();
    return () => {
  ...
  return (
    <div className="p-margin-page flex flex-col gap-gutter max-w-[1600px] mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="font-metadata text-xs bg-primary/10 text-primary px-3 py-1.5 rounded-full font-bold">
            Live Telemetry Stream Active · Command HQ
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-gutter">
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-panel-padding flex flex-col justify-between shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            <span className="font-page-title text-3xl font-bold text-primary">{stats.activeInvestigations}</span>
        ...
        <div className="bg-surface-container-lowest rounded-xl border-2 border-secondary-container p-panel-padding flex flex-col justify-between shadow-[0_4px_12px_rgba(38,56,69,0.08)] relative overflow-hidden">
          ...
          <div className="flex items-baseline gap-2 relative z-10">
            <span className="font-page-title text-3xl font-bold text-secondary-container">{stats.priorityNetworksCount}</span>
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-panel-padding flex flex-col justify-between shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            <span className="font-page-title text-3xl font-bold text-primary">{stats.emergingSignals}</span>
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-panel-padding flex flex-col justify-between shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            <span className="font-page-title text-3xl font-bold text-primary">{stats.crossCaseLinks}</span>
      ...
      <div className="grid grid-cols-12 gap-gutter">
        ...
        {/* Right Column: Emerging Signals Stream */}
