...
export const TimelinePage: React.FC = () => {
  ...
  const { currentCase, openEvidenceModal } = useCaseContext();
  ...
  useEffect(() => {
    ...
    async function loadEvents() {
      try {
        const res = await timelineService.getEvents(currentCase?.id || 'CASE-2026-0142');
  ...
  const filteredEvents = events.filter((evt) => selectedTypes.includes(evt.type));
        ...
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            <span className="bg-primary text-on-primary px-2.5 py-0.5 rounded-full font-label-caps text-[10px] uppercase tracking-wider font-bold">
              Case {currentCase?.caseNumber || '2026-0142'}
            </span>
            <span className="text-on-surface-variant font-metadata text-xs">Live Backend Chronology</span>
          </div>
          <h2 className="font-page-title text-2xl font-bold text-primary">Investigation Timeline: {currentCase?.title || 'Operation Shadow'}</h2>
        </div>
        ...
        <div className="flex-1 bg-white border border-outline-variant rounded-xl p-6 relative shadow-sm">
          ...
          {isLoading ? (
            <div className="p-12 text-center text-xs text-outline flex items-center justify-center gap-2">
              ...
              <span>Loading timeline chronology from backend...</span>
            </div>
          ) : filteredEvents.length === 0 ? (
            <div className="p-12 text-center text-xs text-outline">No timeline events matching filter criteria.</div>
          ) : (
